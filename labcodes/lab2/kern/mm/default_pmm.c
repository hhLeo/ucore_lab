...
// 初始化free_list（用于记录空闲内存块）设置nr_free（总空闲块的数量）为0.
static void
default_init(void) {
	// free_list：记录了空闲内存块
    list_init(&free_list);
...
// default_init_memmap：初始化某一空闲块（基址，页号）
/*
1.初始化这一空闲块在memlayout.h里的每一页, 包括:
...
default_init_memmap(struct Page *base, size_t n) {
    ...
    for (; p != base + n; p ++) {
        ...
        p->flags = 0;
        SetPageProperty(p);//add
        p->property = 0;
        set_page_ref(p, 0);
        list_add_before(&free_list, &(p->page_link));//
    }
    base->property = n;	//first block
...
default_alloc_pages(size_t n) {
    ...
    while ((le = list_next(le)) != &free_list) {
        struct Page *p = le2page(le, page_link);
        if (p->property >= n) {
            // 若块的大小满足大于等于n，则找到符合要求的空闲块（PG_reserved =1, PG_property =0），并将该空闲块从列表中取出。
            // 其中，若是p->property > n，需要重新计算剩余块的大小（减n）。
            //page = p;
            //break;
        	int i;
        	for (i = 0; i < n; i++) {
...
default_free_pages(struct Page *base, size_t n) {
    ...
    */
    // 根据需要free的块基址，搜索空闲列表，找到合适的位置并插入。
    // 其中，空闲页块链表按照空闲页块起始地址来排序，形成一个有序的链表。
    while((le=list_next(le)) != &free_list) {
    	p = le2page(le, page_link);
    	if (p > base) break;
    }
    ...
    // 重置fields of pages, 如p->ref, p->flags (PageProperty)等。
    base->flags = 0;
    set_page_ref(base, 0);
    ClearPageProperty(base);
    SetPageProperty(base);
    ...
    // merge low addr or high addr blocks
    p = le2page(le,page_link) ;
    if (base + n == p) {
    	base->property += p->property;
    	p->property = 0;
