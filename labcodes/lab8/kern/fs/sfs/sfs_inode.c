...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2013011331 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
    while (alen + offset < endpos) {
        ...
        if (blkend > endpos) {
        	size = (endpos - alen - offset);
        }
        else {
        	size = (blkend - alen - offset);
        }
        ...
        if (size == SFS_BLKSIZE) {
        	if (ret = sfs_block_op(sfs, buf + alen, ino, 1)) {
        		goto out;
        	}
        }
        else {
        	if (ret = sfs_buf_op(sfs, buf + alen, size, ino, blkoff)) {
        		goto out;
        	}
        }
        alen += size;
